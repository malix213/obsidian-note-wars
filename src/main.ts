...
import {
	SubdividerSettingTab,
	DEFAULT_SETTINGS as DEFAULT_SUBDIVIDER_SETTINGS,
	type SubdividerSettings,
} from "./subdivider/settings";
import { handle_selection, handle_file } from "./subdivider/handles";
import { TFile } from "obsidian";
import { addAutoIncrementMenuItems } from "./autoincrement-note";
import { TimestampView, TIMESTAMP_VIEW_TYPE } from "./timestamp-scanner/TimestampView";
import { scanForTimestamps } from "./timestamp-scanner/scanner";
...
export default class IdeaEmergencePlugin extends Plugin {
	...
	async onload() {
		...
		// Register the new timestamp view
		this.registerView(
			TIMESTAMP_VIEW_TYPE,
			(leaf) => new TimestampView(leaf, this),
		);
		...
		// Register timestamp scanner command
		this.addCommand({
			id: "scan-timestamps",
			name: "Scan timestamps in current note",
			callback: async () => {
				await this.scanCurrentNoteForTimestamps();
			},
		});
		...
		this.registerEvent(
			this.app.workspace.on("editor-menu", (menu) => {
				menu.addSeparator();
				menu.addItem((item) => {
					item.setTitle("Subdivide the selection")
						.setIcon("blocks")
						.onClick(async () => {
							const selectedText =
		...
		this.registerEvent(
			this.app.workspace.on("file-menu", (menu, file) => {
				...
				if (file instanceof TFile) {
					...
					menu.addItem((item) => {
						item.setTitle("Scan timestamps")
							.setIcon("clock")
							.onClick(async () => {
								await this.scanFileForTimestamps(file);
							});
					});
				}
			}),
	...
	async loadSettings() {
		this.settings = Object.assign(
			{},
			DEFAULT_SETTINGS,
			(await this.loadData()) as Partial<IdeaEmergenceSettings>,
		);
	}
		...
		new PluginSelectionModal(this.app, (selectedPlugins) => {
			void (async () => {
				if (exists) {
					...
					await this.reconfigureVault(
						targetConfigPath,
						selectedPlugins,
					);
				} else {
					new Notice("Initializing new vault...");
					await this.initializeNewVault(
						normalizedPath,
						targetConfigPath,
						selectedPlugins,
					);
	...
	isVaultRegistered(vaultPath: string): boolean {
		try {
			...
			return Object.values(config.vaults).some(
				(v) =>
					path.normalize(v.path).normalize("NFC") ===
					path.normalize(vaultPath).normalize("NFC"),
			);
		} catch {
			return false;
		}
	...
	checkRegistrationStatus() {
		...
		try {
			const content = fs.readFileSync(configPath, "utf8");
			const config = JSON.parse(content) as ObsidianConfig;
			const vaultCount = config.vaults
				? Object.keys(config.vaults).length
				: 0;
			...
			if (this.app.vault.adapter instanceof FileSystemAdapter) {
				...
				console.debug("Registration status:", {
					configPath,
					vaultCount,
					isRegistered,
					id,
	...
	async reconfigureVault(
		targetConfigPath: string,
		selectedPlugins: string[],
	) {
		try {
			const adapter = this.app.vault.adapter as FileSystemAdapter;
			const currentPluginsPath = path.join(
				adapter.getBasePath(),
				this.app.vault.configDir,
				"plugins",
			);
			...
			for (const plugin of installedPlugins) {
				if (!selectedPlugins.includes(plugin)) {
					...
					console.debug(
						`Removed plugin during reconfiguration: ${plugin}`,
					);
				}
			}
			...
			if (fs.existsSync(targetAppJsonPath)) {
				try {
					appConfig = JSON.parse(
						fs.readFileSync(targetAppJsonPath, "utf8"),
	...
	async initializeNewVault(
		...
		if (fs.existsSync(currentConfigPath)) {
			...
			try {
				await Promise.resolve(
					this.copyRecursiveSync(currentConfigPath, targetConfigPath),
				);
				...
				if (fs.existsSync(targetPluginsPath)) {
					...
					for (const plugin of installedPlugins) {
						if (!selectedPlugins.includes(plugin)) {
							...
							console.debug(
								`Removed unselected plugin: ${plugin}`,
							);
						}
					}
				}
				...
				console.error(
					"Failed to copy config or filter plugins:",
					message,
				);
		...
		try {
			...
			if (fs.existsSync(targetAppJsonPath)) {
				...
				try {
					...
					console.error(
						"Failed to parse existing app.json, creating new one.",
						parseError,
					);
				}
			}
	...
	relaunchApp() {
		try {
			...
			const electron = win.require
				? (win.require("electron") as ElectronModule)
				: null;
	...
	spawnNewInstance(normalizedPath: string, delaySeconds: number = 0) {
		...
		if (delaySeconds > 0) {
			if (platform() === "linux" || platform() === "darwin") {
				...
				finalArgs = [
					"-c",
					`sleep ${delaySeconds} && ${command} ${shellEscapedArgs}`,
				];
				...
				finalArgs = [
					"/c",
					`timeout /t ${delaySeconds} /nobreak && ${command} ${shellEscapedArgs}`,
				];
			}
		}
		...
		try {
			console.debug(
				`Spawning Obsidian with command: ${finalCommand} ${finalArgs.join(" ")}`,
			);
			...
			if (platform() === "linux" && delaySeconds === 0) {
				child.on("error", (err) => {
					console.warn(
						`Primary Linux spawn (${command}) failed, trying xdg-open fallback`,
						err,
					);
					const fallback = spawn("xdg-open", [uri], {
						detached: true,
						stdio: "ignore",
					});
					fallback.unref();
	...
	registerVault(vaultPath: string) {
		try {
			...
			if (!configPath || !fs.existsSync(configPath)) {
				console.warn(
					"Obsidian config file not found, skipping registration.",
				);
				return;
			}
	...
	async scanCurrentNoteForTimestamps() {
		const activeFile = this.app.workspace.getActiveFile();
		if (!activeFile) {
			new Notice("No active file to scan");
			return;
		}
		await this.scanFileForTimestamps(activeFile);
	}

	async scanFileForTimestamps(file: TFile) {
		try {
			const content = await this.app.vault.read(file);
			const timestamps = scanForTimestamps(content);

			// Activate or reveal the timestamp view
			const leaves = this.app.workspace.getLeavesOfType(TIMESTAMP_VIEW_TYPE);
			...
			console.error("Failed to scan timestamps:", error);
			new Notice("Failed to scan timestamps");
		}
	}
...
class AlreadyRegisteredModal extends Modal {
	...
	onOpen() {
		...
		contentEl.createEl("p", {
			text: "This directory is already set as a vault. What would you like to do?",
		});
		...
		new Setting(btnContainer)
			...
			.addButton((btn) =>
				btn.setButtonText("Reconfigure").onClick(() => {
...
class PathModal extends Modal {
	...
	onOpen() {
		...
		new Setting(contentEl)
			...
			.setDesc(
				"Absolute path to the directory you want to open as a vault.",
			)
			.addText((text) =>
				text
...
class PluginSelectionModal extends Modal {
	...
	onOpen() {
		...
		warning.createEl("p", {
			text: "This directory is not an Obsidian vault. Initializing it will:",
		});
		const ul = warning.createEl("ul");
		ul.createEl("li", { text: "Copy config from current vault" });
		ul.createEl("li", {
			text: "Relaunch Obsidian actual vault and new vault of directory selected",
		});
		ul.createEl("li", {
			text: "Don't forget to trust and enable plugins in new vault",
		});
		...
		if (fs.existsSync(pluginsPath)) {
			try {
				...
				contentEl.createEl("p", {
					text: "Could not read plugins directory.",
				});
			}
		}
